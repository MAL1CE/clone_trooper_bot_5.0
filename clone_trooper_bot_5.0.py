...
def MemCleanup():
    with open('lists/saved_comment_list.txt', 'r') as scl:
        ...
        mem_list = scl.read().splitlines()
        mem_list_cleaned = [q for q in mem_list if todays_date in q]
        mem_list_cleaned_str = str(mem_list_cleaned).replace('[', '').replace(']', '').replace('"', '').replace("'", '').replace(", ", "\n")
        scl.close()
    if mem_list != mem_list_cleaned:
        with open('lists/saved_comment_list.txt', 'w') as scl:
            scl.write(mem_list_cleaned_str + "\n")
            scl.truncate()
            print("Memory Cleaned!")
            scl.close()
    else:
        print("Memory Already Cleaned, Skipping Process")

#BEGINNING OF WHILE LOOP!
while True:
    
    try:
        clone_list, clone_triggers, follow_orders_triggers, prisoners_triggers, the_creeps_triggers, expendable_triggers, lightsaber_triggers, why_triggers, war_triggers, programmed_triggers, kill_count_triggers, trandoshan_triggers, geonosis_triggers, redredgreen_triggers, egg_triggers, good_bot_triggers, bomb_triggers, bad_bot_triggers, skirata_triggers, sentient_triggers, order_66_triggers, order_67_triggers, order_69_triggers, stormtrooper_filter, bark_triggers, can_the_chatter_triggers, permission_to_speak = Get_Triggers()
        ...
        for comment in reddit.subreddit('PrequelMemes+botmakers_guild').stream.comments(skip_existing=True):
            ...
            with open('lists/chatter_list.txt', 'r') as chl:
                chatter_list = chl.read().splitlines()
                chl.close()
            ...
            with open('lists/ignore_list.txt', 'r')as rf: # Opens ignore_list in read only mode
                ...
                if author_id not in rf_contents and author_id != bot_id: #Checks comment against ignore list and bot id
                    ...
                    if "!ignore" in comment_lower and len(comment_lower) < 10: # Looks for the word "ignore" in the comment and checks length of comment to prevent misfire.
                        ...
                        if is_graylisted == False and author_name not in chatter_list: #Begin Trigger Search and Quote Selection
                            
                            if is_cake_day == True: #Checks for keywords in comment
                                Quote_Selection_Keyword_Priority('responses/cake_day_responses.txt')
                                with open('lists/cake_day_list.txt', 'a') as f: # Opens ignore list in append mode
                                    ...
                                    print(" ")
                                    print("User added to Cake Day List")
                                    print(" ")
                            ...
                            #permission to speak
                            ...
                            elif any(word in comment_lower for word in can_the_chatter_triggers)and comment.parent().author.id == bot_id:
                                with open('lists/chatter_list.txt', 'a') as chl:
                                    chl.write(author_name + "\n")
                                    chl.close()
                                print("===== Generating Reply =====", "\n")
                                comment.reply('"Sorry, Boss... " -Scorch, Delta 62') # Replies to comment with random quote
                                print('"Sorry, Boss... " -Scorch, Delta 62', "\n") # Prints random quote from reply
                                print("===== Reply Posted ======", "\n")
                                ...
                                elif any(word in comment_lower for word in prisoners_triggers):
                                    Quote_Selection_Keyword_Priority('responses/prisoners_responses.txt')
                                ...
                                elif any(word in comment_lower for word in trandoshan_triggers):
                                    Quote_Selection_Keyword_Priority('responses/trandoshan_responses.txt')
                                ...
                                elif clone_detected == True and (comment.parent().author.id == bot_id):
                                    Quote_Selection('responses/clone_responses.txt')
                        ...
                        elif author_name in chatter_list and (any(word in comment_lower for word in clone_triggers) or comment.parent().author.id == bot_id):
                            if any(word in comment_lower for word in permission_to_speak):
                                print(chatter_list)
                                new_chatter_list = [n for n in chatter_list if n != author_name]
                                print(new_chatter_list)
                                new_chatter_list_str = str(new_chatter_list).replace('[', '').replace(']', '').replace('"', '').replace("'", '').replace(", ", "\n")
                                print(new_chatter_list_str)
                                with open('lists/chatter_list.txt', 'w') as chl:
                                    chl.write(new_chatter_list_str + "\n")
                                    chl.truncate()
                                ...
                                if roll_die_string == "1":
                                    print("Step 6")
                                    print("===== Generating Reply =====", "\n")
                                    comment.reply('"Permission to speak, sir?"') # Replies to comment with random quote
                                    print('"Permission to speak, sir?"', "\n") # Prints random quote from reply
                                    print("===== Reply Posted ======", "\n")
                                    time.sleep(cooldown) # Cooldown in seconds
